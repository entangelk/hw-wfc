...
| Exhaustive baseline 대비 품질 | 100% (전환 비용 포함, 레이어당 top-8) |
| WFC 스케줄링 시간 | ~3ms (6-layer attention block, `stress_gpu`) |
| Exhaustive baseline | 262K 조합, ~7-9초 |
| 속도 향상 | **~2400배** |

이 수치는 `python examples/attention_exhaustive_benchmark.py`로 재현됩니다.
여기서 exhaustive baseline은 hard constraint 이후 각 레이어의 상위 8개 후보만 전수 탐색한 값(`8^6 = 262,144`)이며, 전체 후보 카르테시안 곱 전체를 돈 것은 아닙니다. 따라서 절대 시간은 실행 환경에 따라 달라질 수 있습니다.
추가로 `stress_gpu`와 `toy_gpu`에서 `top-k = 4/6/8/10`까지 반복 검증했을 때도 최종 선택은 동일했고, 전환 비용 포함 품질은 계속 `100%`였습니다.
...
RTX 3060 (SM86, CUDA 12.8)에서 codegen 생성 Triton 커널 검증:
...
| 워크로드 | WFC (TFLOPS) | Autotuner (TFLOPS) | 비율 |
|----------|-------------|-------------------|------|
...
**평균: 90%** — 제약 전파 ~1ms로 달성 vs autotuner의 수십 회 커널 실행.
